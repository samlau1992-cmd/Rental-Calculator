...
# --- STYLING (Condensed & Printer Friendly) ---
st.markdown("""
    <style>
    ...
    @media print {
        ...
        .stApp { zoom: 70%; background-color: white !important; }
        [data-testid="stSidebar"] { display: none !important; }
        .main .block-container { max-width: 100% !important; padding: 0 !important; margin: 0 !important; }
        .stTable, .excel-header, .stMetric { page-break-inside: avoid !important; }
...
def calculate_ontario_ltt(price, is_first_buyer, city):
    tax = 0
    ...
    if city == "Toronto":
        m_tax = 0
        if price > 2000000: m_tax += (price - 2000000) * 0.025 + 36475
        elif price > 400000: m_tax += (price - 400000) * 0.02 + 4475
        elif price > 250000: m_tax += (price - 250000) * 0.015 + 2225
        elif price > 55000: m_tax += (price - 55000) * 0.01 + 275
        else: m_tax += price * 0.005
...
if st.session_state.page == 'home':
    st.markdown("<h1 style='text-align: center; margin-top: 50px;'>HIA COMMAND CENTER</h1>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    if c1.button("📊 MARKET INTELLIGENCE", use_container_width=True): 
        change_page('bdwm')
    if c2.button("🏠 RENTAL ANALYSIS", use_container_width=True): 
        change_page('rental')
    ...
    with st.sidebar:
        ...
        city = st.selectbox("City", ["Windsor", "Toronto", "Ottawa", "London", "Kitchener", "Hamilton", "Other"])
    ...
    # --- MATH ENGINE ---
    ltt_val = calculate_ontario_ltt(p_price, is_first_buyer, city)
    
    if down_pct >= 0.20:
        cmhc_p = 0.0
    else:
        cmhc_p = 0.04 if down_pct <= 0.05 else 0.031 if down_pct <= 0.1 else 0.028 if down_pct <= 0.15 else 0.024
        
    loan_amt = (p_price * (1 - down_pct)) * (1 + cmhc_p)
    down_amt = p_price * down_pct
    invested_cap = (down_amt + legal_closing + ltt_val + renos + other_upfront) - heloc_used

    def calc_stats(rate, rent):
        i = rate / 12
    ...
    st.title(f"🏠 Rental Analysis: {city}")
    ...
    at_net_rows = []
    for r in rent_steps:
        _, _, _, _, pocket_money = calc_stats(m_rate, r)
    ...
    for s in shifts:
        cash, prin, at, bt, _ = calc_stats(m_rate + s, target_rent)
        
        if bt <= 0:
            repay_val = "99.00+"
        elif invested_cap <= 0:
            repay_val = "0.00 (Instant)"
        else:
            repay_val = f"{invested_cap/bt:.2f}"

        matrix_df[f"{s*100:+.1f}%"] = [
            f"{s*100:+.1f}%", f"{(m_rate+s)*100:.2f}%", f"${cash:,.0f}", f"${prin:,.0f}",
            f"${at:,.0f}", f"${bt:,.0f}", f"{(cash/invested_cap)*100:.2f}%",
            f"{(prin/invested_cap)*100:.2f}%", f"{(at/invested_cap)*100:.2f}%", f"{(bt/invested_cap)*100:.2f}%", repay_val
        ]
    
    st.table(matrix_df.style.apply(lambda row: ['background-color: yellow;' if row.name in [4, 5, 8, 9] else '' for _ in row], axis=1))

    # --- PAGE 2: PRINT ONLY SUMMARY ---
    st.markdown("<div class='page-break print-only-summary'></div>", unsafe_allow_html=True)
    st.markdown("<div class='print-only-summary'>", unsafe_allow_html=True)
    st.markdown("<div class='excel-header'>ANALYSIS INPUT SUMMARY</div>", unsafe_allow_html=True)
    
    summary_df = pd.DataFrame({
        "Variable": ["City", "Purchase Price", "Downpayment %", "Mortgage Rate", "HELOC Rate", "Amortization", "Marginal Tax", "Target Rent", "Property Tax", "Insurance", "Equip Rental", "Renovations"],
        "Value": [city, f"${p_price:,.0f}", f"{down_pct*100:.1f}%", f"{m_rate*100:.2f}%", f"{h_rate*100:.1f}%", f"{amort} Yrs", f"{tax_bracket*100:.2f}%", f"${target_rent:,.0f}", f"${prop_tax:,.0f}", f"${ins:,.0f}", f"${equip_rent:,.0f}", f"${renos:,.0f}"]
    })
    st.table(summary_df)
